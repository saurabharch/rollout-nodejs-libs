...
describe('RolloutChatbotUtil', function() {
    describe('findSplits()', function() {
        it('should return all the splitted messages', function() {
            ...
            var commands = RolloutChatbotUtil.findSplits(text);
            assert(commands != null);
            assert(commands.length == 5);
            console.log("Commands:", commands)
            assert.strictEqual(commands[0].type, RolloutChatbotUtil.COMMAND_TYPE_MESSAGE);
            assert.strictEqual(commands[0].text, 'Intro text');
            assert.strictEqual(commands[1].type, RolloutChatbotUtil.COMMAND_TYPE_WAIT);
            assert.strictEqual(commands[1].time, 2000);
            assert.strictEqual(commands[2].type, RolloutChatbotUtil.COMMAND_TYPE_MESSAGE);
            assert.strictEqual(commands[2].text, 'Text2');
            assert.strictEqual(commands[3].type, RolloutChatbotUtil.COMMAND_TYPE_WAIT);
            assert.strictEqual(commands[3].time, 1000);
            assert.strictEqual(commands[4].type, RolloutChatbotUtil.COMMAND_TYPE_MESSAGE);
...
describe('RolloutChatbotUtil', function() {
    describe('splitMessage()', function() {
        it('should return multiple commands', function() {
            const text = "Intro text\n-\nsecond message\n-\nthird message";
            console.log("splitting text:", text);
            var commands = RolloutChatbotUtil.splitMessage(text);
            console.log("Commands:", commands)
            assert(commands != null);
            assert(commands.length == 5);
            ...
            assert.strictEqual(commands[0].text, 'Intro text');
            assert.strictEqual(commands[1].type, RolloutChatbotUtil.COMMAND_TYPE_WAIT);
            assert.strictEqual(commands[1].time, 500);
            assert.strictEqual(commands[2].type, RolloutChatbotUtil.COMMAND_TYPE_MESSAGE);
            assert.strictEqual(commands[2].text, 'second message');
            assert.strictEqual(commands[3].type, RolloutChatbotUtil.COMMAND_TYPE_WAIT);
            assert.strictEqual(commands[3].time, 500);
            assert.strictEqual(commands[4].type, RolloutChatbotUtil.COMMAND_TYPE_MESSAGE);
...
describe('RolloutChatbotUtil', function() {
    describe('splitMessage()', function() {
        it('should return multiple commands, first is type WAIT', function() {
            ...
            assert.strictEqual(commands[0].type, RolloutChatbotUtil.COMMAND_TYPE_WAIT);
            assert.strictEqual(commands[0].time, 500);
            assert.strictEqual(commands[1].type, RolloutChatbotUtil.COMMAND_TYPE_MESSAGE);
            assert.strictEqual(commands[1].text, 'Intro text');
            assert.strictEqual(commands[2].type, RolloutChatbotUtil.COMMAND_TYPE_WAIT);
            assert.strictEqual(commands[2].time, 500);
            assert.strictEqual(commands[3].type, RolloutChatbotUtil.COMMAND_TYPE_MESSAGE);
