...
static is_agent_handoff_command(msg) {
  ...
  const text = msg.text;
  const agent_pattern = new RegExp('^(' + RolloutChatbotUtil.AGENT_DIRECTIVE.replace(/\\/g, '\\\\') + ')$', 'm');
  ...
  if (match_agent && match_agent.length >=2) {
    ...
    return {
      'agent_handoff': RolloutChatbotUtil.AGENT_DIRECTIVE,
      'text': new_msg_text
    }
  }
  return {
 ...
 */
  ...
  */
  ...
  static parseReply(text) {
      ...
      reply.message[RolloutChatbotUtil.TEXT_KEY] = text
      reply.message[RolloutChatbotUtil.TYPE_KEY] = RolloutChatbotUtil.TYPE_TEXT

      let parsed;
      parsed = RolloutChatbotUtil.parse_legacy_image(text, reply);
      parsed = RolloutChatbotUtil.parse_tdImage(parsed.text, parsed.reply);
      parsed = RolloutChatbotUtil.parse_tdFrame(parsed.text, parsed.reply);
      parsed = RolloutChatbotUtil.parse_tdVideo(parsed.text, parsed.reply);
      // parsed = RolloutChatbotUtil.parse_bullet_buttons(parsed.text, parsed.reply);
      parsed = RolloutChatbotUtil.parse_tdButtons(parsed.text, parsed.reply);
      parsed = RolloutChatbotUtil.parse_webhook_with_url(parsed.text, parsed.reply);
      parsed = RolloutChatbotUtil.parse_webhook_on_off(parsed.text, parsed.reply);
  ...
  static parse_button_from_string(button_string) {
    ...
    const tdaction_tag = RolloutChatbotUtil.ACTION_TAG; // 'tdAction:';
    const tdaction_pattern = new RegExp('(' + tdaction_tag + ')(\\S+)', 'm');
    
    const tdaction_show_echo_tag = RolloutChatbotUtil.ACTION_SHOW_ECHO_TAG;
    const tdaction_show_echo_pattern = new RegExp('(' + tdaction_show_echo_tag + ')(\\S+)', 'm');

    const tdintent_tag = RolloutChatbotUtil.INTENT_TAG; // 'tdIntent:';
    const tdintent_pattern = new RegExp('(' + tdintent_tag + ')(\\S+)', 'm');

    const tdintent_no_echo_tag = RolloutChatbotUtil.INTENT_NO_ECHO_TAG;
    ...
    if (match_button_action && match_button_action.length && match_button_action.length === 3) {
      ...
      const button =  RolloutChatbotUtil.create_action_button_by_match(button_string, match_button_action, show_echo);
      return button;
    }
      ...
      const button =  RolloutChatbotUtil.create_action_button_by_match(button_string, match_button_action_show_echo, show_echo);
      return button;
    }
    else if (match_button_intent && match_button_intent.length && match_button_intent.length === 3) {
      const show_echo = true;
      const button =  RolloutChatbotUtil.create_action_button_by_match(button_string, match_button_intent, show_echo);
      return button;
    }
    else if (match_button_intent_no_echo && match_button_intent_no_echo.length && match_button_intent_no_echo.length === 3) {
      const show_echo = false;
      const button =  RolloutChatbotUtil.create_action_button_by_match(button_string, match_button_intent_no_echo, show_echo);
      return button;
    }
    else if (match_button_link_parent && match_button_link_parent.length && match_button_link_parent.length === 3) {
      const button =  RolloutChatbotUtil.create_link_button_by_match(button_string, match_button_link_parent, RolloutChatbotUtil.TARGET_BUTTON_LINK_PARENT);
      return button;
    }
    else if (match_button_link_self && match_button_link_self.length && match_button_link_self.length === 3) {
      const button =  RolloutChatbotUtil.create_link_button_by_match(button_string, match_button_link_self, RolloutChatbotUtil.TARGET_BUTTON_LINK_SELF);
      return button;
    }
    else if (match_button_link && match_button_link.length && match_button_link.length === 3) {
      const button =  RolloutChatbotUtil.create_link_button_by_match(button_string, match_button_link, RolloutChatbotUtil.TARGET_BUTTON_LINK_BLANK);
      return button;
    }
    else {
      // No subpatterns = text button
      let button = {};
      button[RolloutChatbotUtil.TYPE_KEY] = RolloutChatbotUtil.TYPE_TEXT;
      button[RolloutChatbotUtil.VALUE_KEY] = button_string;
      return button;
    }
    return button
  }
  ...
  static create_link_button_by_match(button_string, match, target) {
    ...
    button[RolloutChatbotUtil.TYPE_KEY] = RolloutChatbotUtil.TYPE_BUTTON_URL;
    button[RolloutChatbotUtil.VALUE_KEY] = button_label;
    button[RolloutChatbotUtil.LINK_KEY] = link;
    button[RolloutChatbotUtil.TARGET_KEY] = target;
    return button;
  }
  ...
  static create_action_button_by_match(button_string, match, show_echo) {
    ...
    const button_label = button_string.replace(command,'').trim();
    button[RolloutChatbotUtil.TYPE_KEY] = RolloutChatbotUtil.TYPE_BUTTON_ACTION;
    button[RolloutChatbotUtil.VALUE_KEY] = button_label;
    button[RolloutChatbotUtil.ACTION_KEY] = action;
    button[RolloutChatbotUtil.SHOW_ECHO_KEY] = show_echo;
    return button;
  }
  ...
  static parse_legacy_image(text, reply) {
    ...
    if (images && images.length > 0) {
      ...
      reply.message[RolloutChatbotUtil.TEXT_KEY] = text
      reply.message[RolloutChatbotUtil.TYPE_KEY] = RolloutChatbotUtil.TYPE_IMAGE
      reply.message[RolloutChatbotUtil.METADATA_KEY] = {
        src: image_url,
        width: width,
        height: height
      }
  ...
  static parse_tdImage(text, reply) {
    const image_tag = RolloutChatbotUtil.IMAGE_TAG; // 'tdImage';
    ...
    if (tdimages_match && tdimages_match.length === 3) {
      ...
      if (width) {
        reply.message[RolloutChatbotUtil.METADATA_KEY].width = width;
      }
      if (height) {
        reply.message[RolloutChatbotUtil.METADATA_KEY].height = height;
      }
    }
    return {
      text: text,
  ...
  static parse_tdFrame(text, reply) {
    const frame_tag = RolloutChatbotUtil.FRAME_TAG; //'tdFrame';
    ...
    if (tdframes_match && tdframes_match.length === 3) {
      ...
      reply.message[RolloutChatbotUtil.TEXT_KEY] = text
      reply.message[RolloutChatbotUtil.TYPE_KEY] = RolloutChatbotUtil.TYPE_FRAME
      reply.message[RolloutChatbotUtil.METADATA_KEY] = {
        src: frame_url
      }
      if (width) {
  ...
  static parse_tdVideo(text, reply) {
    ...
    // const tdImage_pattern = new RegExp('^(' + image_tag + '.*):(http(?:s)*.*)', 'm');
    const tdvideo_tag = RolloutChatbotUtil.VIDEO_TAG; // 'tdVideo';
    ...
    if (tdvideos_match && tdvideos_match.length === 3) {
      ...
      video_url = RolloutChatbotUtil.check_is_youtube_video_url(video_url);
      ...
      reply.message[RolloutChatbotUtil.METADATA_KEY][RolloutChatbotUtil.FRAME_TYPE_KEY] = RolloutChatbotUtil.TYPE_VIDEO;
      if (width) {
        reply.message[RolloutChatbotUtil.METADATA_KEY].width = width;
      }
      if (height) {
  ...
  //     reply.message[RolloutChatbotUtil.TEXT_KEY] = text_with_removed_buttons
  //     // estrae i bottoni
  //     let buttons = []
  ...
  //       let button = RolloutChatbotUtil.parse_button_from_string(button_text);
  //       // var button = {}
  //       // button[TYPE_KEY] = "text"
  //       // button["value"] = button_text
  //       buttons.push(button)
  //     });
  //     if (reply.message[RolloutChatbotUtil.ATTRIBUTES_KEY] == null) {
  //       reply.message[RolloutChatbotUtil.ATTRIBUTES_KEY] = {}
  //     }
  //     reply.message[RolloutChatbotUtil.ATTRIBUTES_KEY]["attachment"] = {
  //       type:"template",
  //       buttons: buttons
  //     }
  ...
  static parse_tdButtons(text, reply) {
    const tdbutton_tag = RolloutChatbotUtil.BUTTON_TAG;// 'tdButton:';
  ...
  static parse_webhook_with_url(text, reply) {
    ...
    if (webhooks && webhooks.length > 0) {
      ...
      // reply.message[RolloutChatbotUtil.WEBHOOK_KEY] = webhook_url;
      reply.message[RolloutChatbotUtil.TEXT_KEY] = text;
...
module.exports = { RolloutChatbotUtil };