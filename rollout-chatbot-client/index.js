...
const { RolloutClient } = require('@rollout/rollout-client');

/**
 * This is the class that handles the communication with Rollout's APIs
 */
class RolloutChatbotClient {

  /**
   * Constructor for RolloutClient object
   *
   * @example
   * const { RolloutChatbotClient } = require('rollout-chatbot-client');
   * const tdclient = new RolloutChatbotClient({request: request, response: response});
   *
   * @param {Object} options JSON configuration.
   * @param {Object} options.request Optional. Express HTTP request object.
   * @param {Object} options.APIKEY Mandatory. Rollout APIKEY.
   * @param {Object} options.APIURL Optional. Rollout server api endpoint. If not provided, cloud endpoint is used
  ...
  constructor(options) {
    ...
    // now initialize an instance of RolloutClient:
    this.RolloutClient = new RolloutClient(
    {
  ...
  sendMessage(msg, callback) {
    this.RolloutClient.sendSupportMessage(
      this.requestId,
      msg,
      callback);
  }
  ...
  updateRequest(properties, attributes, callback) {
    this.RolloutClient.updateRequest(
      this.requestId,
      properties,
      attributes,
      callback);
  ...
  updateDepartment(dep_id, callback) {
    this.RolloutClient.updateDepartment(
      this.requestId,
      dep_id,
      callback);
  }
  ...
  updateLeadEmailFullname(email, fullname, callback) {
    ...
    this.RolloutClient.updateLeadEmailFullname(
      this.leadId,
      email,
      fullname,
      callback);
...
module.exports = { RolloutChatbotClient };