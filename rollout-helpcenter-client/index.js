...
class HelpCenterQuery {
  static DEFAULT_API_ENDPOINT = "https://rollout-cms-server-prod.herokuapp.com";
  ...
  constructor(options) {
    if (!options) {
      throw new Error(
        "options.APIKEY, options.projectId and options.token are mandatory."
      );
    }
  ...
  search(text, maxResults, callback) {
    ...
    var url =
      this.APIURL +
      `/${this.projectId}/${this
        .workspaceId}/contents/search?text=${escaped_text}&maxresults=${maxResults}`;
